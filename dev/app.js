...
const SESSION_USER_KEY='qas2_current_user';
const SESSION_LOGIN_KEY='qas2_logged_in';
const SESSION_LAST_TAB_KEY='qas2_last_active_tab';
...
function saveSession(user){
  try{
    ...
    localStorage.setItem(SESSION_LOGIN_KEY, '1');
  }catch(err){
...
function clearSession(){
  try{
    ...
    localStorage.removeItem(SESSION_LAST_TAB_KEY);
  }catch(err){
...
function getSavedSession(){
  try{
    ...
    const logged = localStorage.getItem(SESSION_LOGIN_KEY);
    if(logged !== '1' || !raw) return null;
    return JSON.parse(raw);
  }catch(err){
    console.error('getSavedSession failed:', err);
    return null;
  }
}

function saveLastActiveTab(tabName){
  try{
    localStorage.setItem(SESSION_LAST_TAB_KEY, tabName);
  }catch(err){
    console.error('saveLastActiveTab failed:', err);
  }
}

function getLastActiveTab(){
  try{
    return localStorage.getItem(SESSION_LAST_TAB_KEY) || (isAdmin() ? 'dashboard' : (isSupervisor() ? 'register' : 'new'));
  }catch(err){
    console.error('getLastActiveTab failed:', err);
    return isAdmin() ? 'dashboard' : (isSupervisor() ? 'register' : 'new');
  }
}
...
function showApp(){
  ...
  if(adminMode){rb.style.display='inline-block';rb.style.background='#C00000';rb.textContent='ADMIN';}
  else if(supervisorMode){rb.style.display='inline-block';rb.style.background='#375623';rb.textContent='SUPERVISOR';}
  else rb.style.display='none';
  document.querySelectorAll('.admin-only').forEach(el=>el.style.display=adminMode?'':'none');
  initForm();
  const preferredTab = getLastActiveTab();
  if(adminMode)switchTab(preferredTab || 'dashboard');
  else if(supervisorMode)switchTab(preferredTab || 'register');
  else switchTab(preferredTab || 'new');
}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const restoredUser = getSavedSession();
  if(restoredUser){
    currentUser = restoredUser;
    try{
      showApp();
    }catch(err){
      console.error('restoreSession/showApp failed:', err);
      currentUser = null;
      clearSession();
      document.getElementById('main-app').style.display='none';
      document.getElementById('login-screen').style.display='flex';
